...
namespace server.Forms
{
    public partial class MainMenu : Form
    {
        ...
        //    string jsonRequest = Encoding.UTF8.GetString(ms.ToArray());
        //    LogMessage($"Received from {clientAddress}: {jsonRequest}");
        //    Logger.Write("RECEIVED FROM CLIENT", $"Received from {clientAddress}: {jsonRequest}");
        ...
        private Packet HandleProductCreate(Packet request)
        {
            try
            {
                LogMessage($"Processing product creation: {request.Data["name"]}");
                ...
                using (var connection = new MySqlConnection(connectionString))
                {
                    ...
                    LogMessage($"Successfully created new product: {name}");
            ...
            {
                LogMessage($"Product creation error: {ex.Message}");
        ...
        {
            // Create a response packet to return validation errors
            ...
            // If there are any validation errors, return the response packet
            if (responsePacket.Data.Count > 0)
            {
                return responsePacket; // Return the packet with validation errors
            }

            // If validation passes, you can modify the request packet if needed
